...
time_to_sleep = 5 # in seconds, how long the fron ewill wait after dropping off a taco

def get_ditance_meters(targetLOcation, currentLOcation):
...
def goto(targetLocation):
    distance_to_targetLocation = get_ditance_meters(targetLocation, vehicle.location.global_relative_frame)
    vehicle.simple_goto(targetLocation)
    while vehicle.mode.name == 'GUIDED':
        currentDistance = get_ditance_meters(targetLocation, vehicle.location.global_relative_frame)
        if currentDistance < distance_to_targetLocation * .02:
...
def msg_receiver(message):
    global not_found_count, found_count, time_last, time_to_wait, id_to_find, sub
    ...
    if time.time() - time_last > time_to_wait:
        ...
        if vehicle.mode != 'LAND':
            ...
            while vehicle.mode != 'LAND':
                time.sleep(1)
            print("Vehicle can go to Land Mode")
...
if __name__ == '__main__':
    try:
        ...
        wp_taco = LocationGlobalRelative(-35.36303741, 149.1652374, takeoff_height)  # this is original waypoint +25 meters
